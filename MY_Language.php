...
class MY_Language extends CI_Language {
	...
	***************************************************/
	...
	// Whether to use standalone hosts to identify current language
	var $host_mode = true;
	...
	function MY_Language()
	{
		...
		if ($this->host_mode)
		{
			$foundLanguageInConfig = false;
			foreach($this->languages as $lang => $language)
			{
				$langHost = $CFG->item("base_url_" . $lang);
				if (substr_count($langHost, $_SERVER["HTTP_HOST"]))
				{
					$CFG->set_item("language", $language);
					$CFG->set_item("base_url", $langHost);
					$foundLanguageInConfig = true;
					break;
				}
			}
			if (!$foundLanguageInConfig)
			{
